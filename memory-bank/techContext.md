...
- `scripts/setup.sh` resets `plugintest/CURRENT/artifacts/` and regenerates `plugintest/CURRENT/fixtures/`; it never touches `plugintest/CURRENT/observations/`. Default work root is `$PWD/plugintest/<UTC-stamp>/` (via `plugintest/CURRENT`). Optional `CONFORMANCE_WORK` overrides that path. `$PLUGIN_ROOT/.conformance-work` is a one-line pointer so LSP can find the cwd run when its process cwd is not the workspace.
